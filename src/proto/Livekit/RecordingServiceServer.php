 ...
 */
final class RecordingServiceServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use RecordingServiceServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/livekit.RecordingService/';
    ...
    {
        return $this->prefix.'/livekit.RecordingService/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withServiceName($ctx, 'RecordingService');
        ...
        if ($service != 'livekit.RecordingService') {
        ...
        switch ($method) {
            case 'StartRecording':
                return $this->handleStartRecording($ctx, $req);
            case 'AddOutput':
                return $this->handleAddOutput($ctx, $req);
            case 'RemoveOutput':
                return $this->handleRemoveOutput($ctx, $req);
            case 'EndRecording':
                return $this->handleEndRecording($ctx, $req);
    ...
    private function handleStartRecording(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleStartRecordingJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleStartRecordingProtobuf($ctx, $req);
                break;
    ...
    private function handleStartRecordingJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'StartRecording');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Livekit\StartRecordingRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->StartRecording($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling StartRecording. null responses are not supported'));
            }
    ...
    private function handleStartRecordingProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'StartRecording');
    ...
    private function handleAddOutput(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleAddOutputJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleAddOutputProtobuf($ctx, $req);
                break;
    ...
    private function handleAddOutputJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'AddOutput');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Livekit\AddOutputRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->AddOutput($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling AddOutput. null responses are not supported'));
            }
    ...
    private function handleAddOutputProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'AddOutput');
    ...
    private function handleRemoveOutput(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleRemoveOutputJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleRemoveOutputProtobuf($ctx, $req);
                break;
    ...
    private function handleRemoveOutputJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'RemoveOutput');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Livekit\RemoveOutputRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->RemoveOutput($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling RemoveOutput. null responses are not supported'));
            }
    ...
    private function handleRemoveOutputProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'RemoveOutput');
    ...
    private function handleEndRecording(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleEndRecordingJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleEndRecordingProtobuf($ctx, $req);
                break;
    ...
    private function handleEndRecordingJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'EndRecording');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Livekit\EndRecordingRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->EndRecording($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling EndRecording. null responses are not supported'));
            }
    ...
    private function handleEndRecordingProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'EndRecording');

...
/**
 * @internal ConnectorAbstractClient provides abstraction for JsonClient and Client (default).
 ...
 */
abstract class ConnectorAbstractClient
{
    /**
     * @var string
     */
     ...
     * {@inheritdoc}
     */
    public function DialWhatsAppCall(array $ctx, \Livekit\DialWhatsAppCallRequest $in): \Livekit\DialWhatsAppCallResponse
    {
        ...
        $out = new \Livekit\DialWhatsAppCallResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/livekit.Connector/DialWhatsAppCall';
        } else {
            $url = $url.'/'.$this->prefix.'/livekit.Connector/DialWhatsAppCall';
        }
     ...
     * {@inheritdoc}
     */
    public function DisconnectWhatsAppCall(array $ctx, \Livekit\DisconnectWhatsAppCallRequest $in): \Livekit\DisconnectWhatsAppCallResponse
    {
        $ctx = Context::withPackageName($ctx, 'livekit');
        $ctx = Context::withServiceName($ctx, 'Connector');
        $ctx = Context::withMethodName($ctx, 'DisconnectWhatsAppCall');

        $out = new \Livekit\DisconnectWhatsAppCallResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/livekit.Connector/DisconnectWhatsAppCall';
        } else {
            $url = $url.'/'.$this->prefix.'/livekit.Connector/DisconnectWhatsAppCall';
        }
     ...
     * {@inheritdoc}
     */
    public function ConnectWhatsAppCall(array $ctx, \Livekit\ConnectWhatsAppCallRequest $in): \Livekit\ConnectWhatsAppCallResponse
    {
        $ctx = Context::withPackageName($ctx, 'livekit');
        $ctx = Context::withServiceName($ctx, 'Connector');
        $ctx = Context::withMethodName($ctx, 'ConnectWhatsAppCall');

        $out = new \Livekit\ConnectWhatsAppCallResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/livekit.Connector/ConnectWhatsAppCall';
        } else {
            $url = $url.'/'.$this->prefix.'/livekit.Connector/ConnectWhatsAppCall';
        }
     ...
     * {@inheritdoc}
     */
    public function AcceptWhatsAppCall(array $ctx, \Livekit\AcceptWhatsAppCallRequest $in): \Livekit\AcceptWhatsAppCallResponse
    {
        $ctx = Context::withPackageName($ctx, 'livekit');
        $ctx = Context::withServiceName($ctx, 'Connector');
        $ctx = Context::withMethodName($ctx, 'AcceptWhatsAppCall');

        $out = new \Livekit\AcceptWhatsAppCallResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/livekit.Connector/AcceptWhatsAppCall';
        } else {
            $url = $url.'/'.$this->prefix.'/livekit.Connector/AcceptWhatsAppCall';
        }
     ...
     * {@inheritdoc}
     */
    public function ConnectTwilioCall(array $ctx, \Livekit\ConnectTwilioCallRequest $in): \Livekit\ConnectTwilioCallResponse
    {
        $ctx = Context::withPackageName($ctx, 'livekit');
        $ctx = Context::withServiceName($ctx, 'Connector');
        $ctx = Context::withMethodName($ctx, 'ConnectTwilioCall');

        $out = new \Livekit\ConnectTwilioCallResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/livekit.Connector/ConnectTwilioCall';
        } else {
            $url = $url.'/'.$this->prefix.'/livekit.Connector/ConnectTwilioCall';
        }
